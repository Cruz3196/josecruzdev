  ...
  return (
    <div id="project" className="py-12 px-4">
      <div className="flex flex-col items-center text-center">
        <h2 className="font-bold text-3xl sm:text-4xl mb-5 sm:mb-5">
          Creative Works
        </h2>
        ...
        {ProjectData.map((project, index) => {
          ...
          return (
            <div
              key={project.id}
              className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center max-w-6xl mx-auto py-5 lg:py-10"
            >
              {/* Text Content: Conditionally set order */}
              <div
                className={`order-2 ${isEvenIndex ? "lg:order-2" : "lg:order-1"} text-left`}
              >
                <h1 className="font-bold text-3xl sm:text-3xl mb-4">
                  {project.title}
                </h1>
                <p className="py-2 text-base leading-7 sm:leading-8 whitespace-pre-line">
                  {shownContent}
                </p>
                <button
              ...
              <div
                className={`order-1 ${isEvenIndex ? "lg:order-1" : "lg:order-2"} flex justify-center`}
              >
                <img
                  src={project.image}
